...
| ⬜ todo | `subscription-plans-billing` | fullstack-engineer | Implement tiered subscription plans (free trial → paid) with Stripe integration for in-app payment. Free tier: limited Gemini calls/day, basic tabs only. Paid tiers: higher rate limits, all integrations, priority coaching. Requires Stripe account + webhook setup. |
...
| ⬜ todo | `integration-paywalls` | fullstack-engineer | Gate third-party integrations (Canvas, Gmail, Withings, GitHub, YouTube, X/Twitter, Twitch) behind paid tiers. Free tier gets no integrations; mid tier gets academic (Canvas + GitHub); top tier gets all. Settings page shows lock icons with upgrade CTA. |
| ⬜ todo | `gemini-call-rate-limiting` | backend-engineer | Add per-user Gemini API rate limiting and quota tracking. Free tier: limited daily Gemini calls (e.g. 20/day). Paid tiers: higher/unlimited quotas. Track usage in storage with daily reset. Show remaining calls count in chat UI. |
